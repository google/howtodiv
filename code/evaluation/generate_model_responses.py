...
def get_prompt_versions(task_name, cur_instructions):
  return [
      {
          'prompt': (
              'Assist the user on how to perform the specified task. Narrate'
              ' only one step at a time in a conversative manner\n'
          ),
          'prompt_version': 'h',
      },
      {
          'prompt': (
              f'You are given the STEPS for {task_name}. Using these steps'
              ' assist the user by describing instructions. Narrate only one'
              f' step at a time. \nSTEPS: {"\n".join(cur_instructions)}\n\n'
          ),
          'prompt_version': 'tsh',
      },
  ]
...
def draft_initial_prompt(how_to_prompt, query):
  return f"""<start_of_turn>user
  {how_to_prompt + query}<end_of_turn>
  <start_of_turn>model
  """
...
def draft_intermediate_prompt(prompt, prev_response, query):
  return (
      f'{prompt}{prev_response}<end_of_turn>\n'
...
subdir_to_task = {
    'repot': 'How to repot a plant',
    'jump_car': 'How to jump start a car',
    'coffee': 'How to make pour over coffee',
    'changing_tire': 'How to change a car tire',
    'oatmeal': 'How to make oatmeal',
    'oatmeal_mod': 'How to make oatmeal',
    'oatmeal_parse': 'How to make oatmeal',
    'pinwheels': 'How to make pinwheels',
    'pinwheels_mod': 'How to make pinwheels',
    'pinwheels_parse': 'How to make pinwheels',
    'quesadilla': 'How to make quesadilla',
    'quesadilla_mod': 'How to make quesadilla',
    'quesadilla_parse': 'How to make quesadilla',
    'tea': 'How to make tea',
    'tea_mod': 'How to make tea',
    'tea_parse': 'How to make tea',
    'coffee': 'How to make pour over coffee',
    'coffee_mod': 'How to make pour over coffee',
    'coffee_parse': 'How to make pour over coffee',
}
...
for testf_idx, test_file in enumerate(test_ids):
  ...
  for prompt_idx in range(len(prompt_versions)):
    ...
    howto_prompt = prompt_versions[prompt_idx]['prompt']
    prompt_version = prompt_versions[prompt_idx]['prompt_version']
    prompt = draft_initial_prompt(howto_prompt, dialogue_data[0][1])

    model_response = sampler.sample(prompt).split('<end_of_turn>')[0]
    if 0 not in logs:
      logs[0] = {'query': dialogue_data[0][1], 'gt': dialogue_data[0][2],
                 f'model_response_{prompt_version}': model_response}
    else:
      logs[0][f'model_response_{prompt_version}'] = model_response

    for i in range(len(dialogue_data)-2):
      prompt = draft_intermediate_prompt(prompt, dialogue_data[i][2],
                                         dialogue_data[i+1][1])
      model_response = sampler.sample(prompt).split('<end_of_turn>')[0]
      print(f'Computed response for {i+1}th turn')
      if (i+1) not in logs:
        logs[i+1] = {'query': dialogue_data[i+1][1],
                     'gt': dialogue_data[i+1][2],
                     f'model_response_{prompt_version}': model_response}
      else:
