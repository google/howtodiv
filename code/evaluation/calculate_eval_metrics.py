...
def evaluate_with_llm_as_judge(q, gt, mr):
  """Evaluates a sample response using an LLM as a judge.

  Args:
    q: The user query.
    gt: The ground truth answer.
    mr: The sample response to evaluate.

  Returns:
    A string containing the LLM's reasoning and score.
  """
  eval_prompt = (
      'You will be given a user Query along with the expected Ground-truth'
      ' Answer and Sample Response. Evaluate the quality of Sample Response by'
      ' outputing a Score between 1 to 5 and a Reasoning explaining the'
      ' rationale briefly in a sentence.\nJudge on the basis of accuracy,'
      ' alignment with Ground-truth Answer, conciseness, accuracy and relevance'
      ' of the Sample Response. Penalize long answers, those that include'
      ' details not part of the Ground-truth Answer by giving a lower'
...
for testf_idx, test_file in enumerate(test_ids):
  ...
  with open(eval_fpath, 'r') as json_file:
    session_data = json.load(json_file)
  score_list[test_file] = {
      'bleu_score': [],
      'llmj_score': [],
      'llmj_reason': [],
      'rouge1': [],
  ...
  for turn_idx in session_data:
    ...
    if int(turn_idx) >= 10:
      break
    query, gt, response1, response2 = (
        turn['query'],
        turn['gt'],
        turn['model_response_h'],
        turn['model_response_tsh'],
    )
    used_response = response1  # change this based on the response needed

    llm_judge_output = evaluate_with_llm_as_judge(
        q=query, gt=gt, mr=used_response
    )
    llmj_score = int(llm_judge_output.split('Score: ')[-1])
    llmj_reason = (
        llm_judge_output.split('Score: ')[0].split('Reasoning: ')[-1].strip()
    )
...
out_fpath = 'HowToDIVEvals/scores_prompt_type1.json'
