...
print(f'Average number of turns per session: {np.mean(turns_list)}, median: {np.median(turns_list)}')
...
taskwise_turn_counter["change tire"] = taskwise_turn_counter.pop("changing_tire")
...
colors = ['#FF9999',  '#FFCC99',  '#99FF99',  '#C2C2F0', '#66B2FF','#FFFFB3','#FFB3E6','#B3FFFF',] # Custom colors for aesthetics
plt.pie(sizes, labels=labels,
        colors=colors, shadow=True,
        autopct='%1.1f%%', startangle=90, radius=1.2, textprops={'fontsize': 18})
plt.title('Distribution of procedural tasks', fontsize=22, y=1.05)
plt.axis('equal')
plt.show()
...
print('Average number of user words per turn: ', sum(user_words) / len(user_words))
print('Average number of expert words per turn: ', sum(expert_words) / len(expert_words))

# Parse vs non-parse conversation comparison
parse_sessions = {'user':[], 'expert':[]}
non_parse_sessions = {'user':[], 'expert':[]}
num_files = {'parse':0, 'non-parse': 0}
num_turns = {'parse': 0, 'non-parse': 0}
for file in file_to_dial_lens:
  if '_parse' in file:
    ...
    non_parse_sessions['user'].extend(file_to_dial_lens[file]['user'])
    non_parse_sessions['expert'].extend(file_to_dial_lens[file]['expert'])
    num_files['non-parse'] +=1
    num_turns['non-parse'] += len(file_to_dial_lens[file]['user'])

print('Parse Sessions')
print(f'Average number of user words: {sum(parse_sessions['user']) / len(parse_sessions['user'])} over {num_files['parse']} recordings and {num_turns['parse']} turns')
print('Non Parse Sessions')
print(f'Average number of user words: {sum(non_parse_sessions['user']) / len(non_parse_sessions['user'])} over {num_files['non-parse']} recordingsand {num_turns['non-parse']} turns')

sns.set_style("whitegrid")
categories = ['Parse Data', 'Normal Data']
values = [3.37, 10.73]
...
ax.set_ylabel("Average user-turn word count", fontsize=16, labelpad=10)
ax.set_title("Comparison of turn lengths", fontsize=17, fontweight='bold', pad=20)

sessions = [180, 327]
turns = [2419, 4217]
for idx, bar in enumerate(bars):
    yval = bar.get_height()
    ax.text(bar.get_x() + bar.get_width()/2, yval + 0.1, f'{round(yval, 1)} words',
            ha='center', va='bottom', fontsize=15)
    ax.text(bar.get_x() + bar.get_width()/2, yval - 1.7, f'{sessions[idx]} sessions',
            ha='center', va='bottom', fontsize=15, color='white')
    ax.text(bar.get_x() + bar.get_width()/2, yval - 2.5, f'{turns[idx]} turns',
            ha='center', va='bottom', fontsize=15, color='white')
...
plot_turn_length_histogram(data = user_words, xlabel = 'Number of words in User turns', title = 'Histogram of User turn lengths')

#Expert turn length histogram
plot_turn_length_histogram(data = user_words, xlabel = 'Number of words in Expert turns', title = 'Histogram of Expert turn lengths')
...
all_durations = [max(1, interval[1]-interval[0]) for interval in all_times if (interval[1]-interval[0] < 120)]


#@title Plot video length histogram
plot_turn_length_histogram(data = all_durations, xlabel = 'Video Duration (in seconds)', title = 'Histogram of video lengths')
